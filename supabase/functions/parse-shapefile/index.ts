...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'initial' || action === 'update_contour') {
      if (!boundaryLayer) throw new Error("A camada de contorno (Polígono) não foi encontrada no arquivo ZIP.");
    }
    if (action === 'initial' || action === 'points_only') {
      if (!pointsLayer) throw new Error("A camada de pontos de amostragem não foi encontrada no arquivo ZIP.");
    }
    ...
    const validationSummary = { boundaryValid: true, pointsInside: 0, pointsOutside: 0, outsideCodes: [] as string[] };
    ...
    if (pointsLayer) {
      pointsLayer.features.forEach((f: any) => {
        ...
        const code = props.ID || props.CODE || props.id || props.name || `P-${Math.random().toString(36).substring(7)}`;
